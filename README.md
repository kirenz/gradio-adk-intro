...
A simple example demonstrating how to build an AI agent using Google's Agent Development Kit (ADK) with a Gradio web interface. The agent can tell the current time by using a custom tool.
...
The agent is powered by Google's Gemini model and has been configured with:
...
The `get_current_time()` function in `time_agent/tools.py` demonstrates:
